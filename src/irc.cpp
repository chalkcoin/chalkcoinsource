...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #chalkTEST0\r");
            Send(hSocket, "WHO #chalkTEST0\r");
        } else {
            // randomly join #chalk00-#chalk99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #chalk%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #chalk%02d\r", channel_number).c_str());
        }
