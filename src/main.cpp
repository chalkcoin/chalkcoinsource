...
uint256 hashGenesisBlock("0x00000bde7129c37d457412703ad91b5e91a08405c130e1b2e4a0b313c315330c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // chalk: starting difficulty is 1 / 2^12
static CBigNum bnInitialDifficulty(~uint256(0) >> 26);
...
const string strMessageMagic = "chalk Signed Message:\n";
...
{
    // chalk: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 1.2667 * 60;
...
{
    // chalk: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "chalk Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1428170345;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2180129;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a563db70303e391b8ce76c9c41a0b9bc468da1177ac1e148fd8e7355785812e"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb9, 0xde }; // chalk: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("chalkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("chalkMiner : ProcessBlock, block not accepted");
    }
